...
def benchmark_multi_gpu_matrix_operations(size=8192, iterations=3):
    """Benchmark matrix operations using multiple GPUs"""
    results = {}
    ...
    if TORCH_AVAILABLE and TORCH_GPU_AVAILABLE:
        ...
        if gpu_count > 1:
            print(f"Testing multi-GPU performance with {gpu_count} GPUs...")
            
            # Create tensors on each GPU
            gpu_tensors = []
            for i in range(gpu_count):
                torch.cuda.set_device(i)
                a_gpu_i = torch.randn(size, size, dtype=torch.float32, device=f'cuda:{i}')
                b_gpu_i = torch.randn(size, size, dtype=torch.float32, device=f'cuda:{i}')
                gpu_tensors.append((a_gpu_i, b_gpu_i))
            ...
            for _ in range(iterations):
                ...
                for i in range(gpu_count):
                    torch.cuda.set_device(i)
                    a_gpu_i, b_gpu_i = gpu_tensors[i]
                    c_gpu_i = torch.mm(a_gpu_i, b_gpu_i)
                    results_gpu.append(c_gpu_i)
            ...
            results['multi_gpu_gflops'] = (2 * size**3 * gpu_count) / (multi_gpu_time / iterations) / 1e9
            results['multi_gpu_speedup'] = results['single_gpu_time'] / results['multi_gpu_time']
            
            # Clean up
            for a_gpu_i, b_gpu_i in gpu_tensors:
                del a_gpu_i, b_gpu_i
...
def benchmark_multi_gpu_memory_bandwidth(size_mb=2048, iterations=10):
    """Benchmark memory bandwidth using multiple GPUs"""
    results = {}
    ...
    if TORCH_AVAILABLE and TORCH_GPU_AVAILABLE:
        ...
        if gpu_count > 1:
            print(f"Testing multi-GPU memory bandwidth with {gpu_count} GPUs...")
            
            # Create tensors on each GPU
            gpu_tensors = []
            for i in range(gpu_count):
                torch.cuda.set_device(i)
                a_gpu_i = torch.randn(size, dtype=torch.float32, device=f'cuda:{i}')
                b_gpu_i = torch.randn(size, dtype=torch.float32, device=f'cuda:{i}')
                gpu_tensors.append((a_gpu_i, b_gpu_i))
            ...
            for _ in range(iterations):
                ...
                for i in range(gpu_count):
                    torch.cuda.set_device(i)
                    a_gpu_i, b_gpu_i = gpu_tensors[i]
                    b_gpu_i = a_gpu_i.clone()
                
                torch.cuda.synchronize()
            
            multi_gpu_time = time.time() - start_time
            results['multi_gpu_copy_time'] = multi_gpu_time / iterations
            results['multi_gpu_copy_bandwidth'] = (size_mb * gpu_count * iterations) / (multi_gpu_time / 1024)  # GB/s
...
def print_multi_gpu_results(system_info, matrix_results, memory_results):
    ...
    print(f"\n{'='*80}")
    print(f"MULTI-GPU BENCHMARK RESULTS")
    print(f"{'='*80}")
...
def main():
    ...
    print("Starting Multi-GPU Benchmark Suite")
    print("This will test matrix operations and memory bandwidth across multiple GPUs")
    print("Comparing single GPU vs multi-GPU performance")
    ...
    print("Use these results to compare single GPU vs multi-GPU performance")
