...
def get_system_info():
    ...
    if TORCH_AVAILABLE and TORCH_GPU_AVAILABLE:
        ...
        print(f"GPU Memory: {info['torch_gpu_memory_gb']:.1f} GB")
    
    return info

def benchmark_large_matrix_operations(size=16384, iterations=2):
    """Benchmark large matrix operations with memory management"""
    results = {}
    ...
    # Use smaller chunks for CPU to avoid memory issues
    chunk_size = min(size, 8192)
    a_cpu = np.random.random((chunk_size, chunk_size)).astype(np.float32)
    b_cpu = np.random.random((chunk_size, chunk_size)).astype(np.float32)
    ...
    results['cpu_matmul_gflops'] = (2 * chunk_size**3) / (cpu_time / iterations) / 1e9
    
    # GPU operations using PyTorch with memory management
    if TORCH_AVAILABLE and TORCH_GPU_AVAILABLE:
        ...
        try:
            ...
            if "out of memory" in str(e):
                print(f"GPU memory insufficient for {size}x{size}, trying smaller size...")
                # Fall back to smaller size
                fallback_size = size // 2
                a_gpu = torch.randn(fallback_size, fallback_size, dtype=torch.float32, device='cuda')
                b_gpu = torch.randn(fallback_size, fallback_size, dtype=torch.float32, device='cuda')
                ...
                gpu_time = time.time() - start_time
                results['gpu_matmul_time'] = gpu_time / iterations
                results['gpu_matmul_gflops'] = (2 * fallback_size**3) / (gpu_time / iterations) / 1e9
                results['matmul_speedup'] = results['cpu_matmul_time'] / results['gpu_matmul_time']
...
def benchmark_memory_bandwidth_large(size_mb=4096, iterations=10):
    """Benchmark memory bandwidth with larger datasets"""
    results = {}
    ...
    if TORCH_AVAILABLE and TORCH_GPU_AVAILABLE:
        ...
        try:
            ...
            if "out of memory" in str(e):
                print(f"GPU memory insufficient for {size_mb}MB, trying smaller size...")
                # Fall back to smaller size
                fallback_size_mb = size_mb // 2
                fallback_size = int(fallback_size_mb * 1024 * 1024 // 4)
                
                a_gpu = torch.randn(fallback_size, dtype=torch.float32, device='cuda')
                b_gpu = torch.randn(fallback_size, dtype=torch.float32, device='cuda')
                ...
                gpu_copy_time = time.time() - start_time
                results['gpu_copy_time'] = gpu_copy_time / iterations
                results['gpu_copy_bandwidth'] = (fallback_size_mb * iterations) / (gpu_copy_time / 1024)  # GB/s
...
def benchmark_compute_intensive_large(size=5000000, iterations=100):
    """Benchmark compute-intensive operations with larger datasets"""
    results = {}
    ...
    if TORCH_AVAILABLE and TORCH_GPU_AVAILABLE:
        ...
        try:
            ...
            if "out of memory" in str(e):
                print(f"GPU memory insufficient for {size} elements, trying smaller size...")
                fallback_size = size // 2
...
def print_results(system_info, matrix_results, memory_results, compute_results):
    """Print formatted results"""
    print(f"\n{'='*80}")
    print(f"LARGE-SCALE GPU/CPU BENCHMARK RESULTS")
    print(f"{'='*80}")
